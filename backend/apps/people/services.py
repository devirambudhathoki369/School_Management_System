"""
Guardian portal access provisioning (§18.1).

The school office hands a guardian their portal credentials in person (or
over a verified phone call) — there is no self-signup, because the school is
the authority on who may see a child's record. Credentials are therefore:

- created by staff holding ``students.manage``;
- a temporary password shown exactly once to the operator, never stored in
  plain text anywhere;
- flagged ``password_change_required`` so the portal forces the guardian to
  set their own secret on first login.
...
def _available_username(guardian: Guardian) -> str:
    base = _username_base(guardian)
    candidate = base if len(base) >= 4 else f"{base}{secrets.randbelow(10_000):04d}"
    for _ in range(50):
        if not Account.objects.filter(role=Role.GUARDIAN, username=candidate).exists():
            return candidate
...
def provision_portal_access(guardian: Guardian) -> tuple[Account, str, bool]:
    ...
    Returns (account, temp_password, created). The temp password exists only
    in this return value — the caller shows it once and discards it.
    """
    ...
    with transaction.atomic():
        ...
        guardian.save(update_fields=["account", "updated_at"])
    return account, temp_password, True


def revoke_portal_access(guardian: Guardian) -> bool:
    """Disable the guardian's login and end their sessions. The account row
