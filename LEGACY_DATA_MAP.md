...
| Legacy table       |    Rows | → Target           | Notes                                                                                                                                                                                                                                                                |
| ------------------ | ------: | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `main_historylog`  | 664,629 | `audit.AuditEvent` | Append-only (**DB trigger rejects UPDATE/DELETE**; monthly partitioning + hash chain deferred until volume demands). Generic (user_type, user_id) → unified account FK via the identity id-map (superadmins + orphaned accounts keep a readable `actor_label`; 2,161 unmatched). **ETL sequencing:** also mined to backfill `created_by` on payments/vouchers _before_ being archived. ✅ 664,629 imported verbatim (original timestamps, changes JSON, IPs; objects addressed as `legacy:<app>.<model>` + legacy id, joinable via `legacy_id` columns); action counts reconcile exactly (526,777 create / 136,569 update / 1,283 delete). |
