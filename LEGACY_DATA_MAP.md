...
| 14 | `inventory` | Categories, items, stock transactions *(exists in prod, absent from docs)* |
...
| `main_staffotherinfo` | 2,411 | `people.StaffProfile` + `payroll.SalaryStructure` | T: split — bio fields to profile; `basic_salary/grade/allowance/extra/insurance/pf_contrib/pan_no` to payroll (salary history becomes versionable). |
...
| `main_studentledger` | 359,109 | `billing.Charge` + `billing.ChargeLine` | **T:** each `details` key → one line. Real title ids → `line_type=FEE` + FK + label from `__titles` snapshot; reserved keys → typed lines: `tn`→TRANSPORT, `od`→OLD_DUES, `ob`→OPENING_BALANCE, `discounts`→DISCOUNT, `lib_fine`→LIBRARY_FINE, `cfo`→CARRY_FORWARD_OUT (negative, never collectable, M7). Skip `__titles` when iterating (M4). DB check: sum(lines) = total. |
| `main_studentinvoice` | 202,039 | `billing.Payment` + `billing.PaymentLine` | **T (object form, 196,349):** per-title `{amt,dis,due,tdsp,tdsa}` → line columns (amount, discount, due_after, tax_pct, tax_amt). **T (array form, 5,690 = cash receipts):** `[{AMT,PAR}]` → free-text lines; payer from `extra_info` → `payer_name`/`payer_address`. **M1 preserved:** `total_paid` is pre-discount — document on the column. Legacy serial → `legacy_serial` (non-unique, finding #1); new payments get transactional serials. `created_by` backfilled from `main_historylog`. Null `class_info` backfilled where derivable (finding #3). |

### 4.9 `payroll`

| Legacy table | Rows | → Target | Notes |
| --- | ---: | --- | --- |
| `main_staffledger` | 736 | `payroll.SalaryAccrual` + lines | Same JSON→lines treatment. |
| `main_staffinvoice` | 543 | `payroll.SalaryPayment` + lines | **M1 asymmetry:** `total_paid` here is post-deduction — normalize by storing `gross`, `deductions`, `net` explicitly so the asymmetry dies in the new schema. |

### 4.10 `accounting`

| Legacy table | Rows | → Target | Notes |
| --- | ---: | --- | --- |
| `accounting_accountingeconomicyear` | 37 | `accounting.FiscalYear` | `previous_ey` chain preserved (year close). |
| `accounting_accountingledger` | 1,444 | `accounting.LedgerAccount` | Group 1–34 → FK to a seeded `LedgerGroup` reference table (natural side, category, cash-flow class). |
| `accounting_accountingledgerbalance` | 1,455 | `accounting.OpeningBalance` | Unique (ledger, fiscal_year). |
| `accounting_{income,expense,journal,contra}entry` | 876 / 1,527 / 2,883 / 0 | `accounting.Voucher(type=…)` | **T: unify 4 entry tables** into one voucher table; per-type serial preserved (`INV-n`/`EXV-n`/`JRV-n`/`CNV-n`) via per-(school, FY, type) counters. `created_by` backfilled from audit log. |
| `accounting_*particular` | 5,014 / 4,233 / 9,180 / 0 | `accounting.VoucherLine` | Every line gets an explicit Dr/Cr side (derived from voucher type for income/expense/contra; from `type` for journal). **DB constraint: voucher balances** (deferred trigger); the 4 unbalanced legacy vouchers imported with `needs_review=true` (finding #2). |
...
| Legacy table | Rows | → Target | Notes |
| --- | ---: | --- | --- |
| `main_historylog` | 664,629 | `audit.AuditEvent` | Append-only, partitioned by month, tamper-evident (hash chain). Generic (user_type, user_id) → unified account FK via the identity id-map. **ETL sequencing:** also mined to backfill `created_by` on payments/vouchers *before* being archived into the new table. |
...
- **M2 must be rewritten:** receipt serials are *intended* to be unique per school+AY but are not in practice; the rebuild makes uniqueness real (new rows only).
